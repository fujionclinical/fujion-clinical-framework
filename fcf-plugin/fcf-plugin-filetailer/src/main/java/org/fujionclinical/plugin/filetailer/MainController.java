...
public class MainController extends PluginController {
    ...
    private void stopTailer() {
        if (isTailerStarted) {
            ...
            btnToggle.setLabel(StrUtil.formatMessage("@fcf.filetailer.btn.toggle.on.label"));
            showMessage("@fcf.filetailer.msg.stopped");
        }
    }
        ...
        if (!isTailerStarted) {
            ...
            if (tailer == null) {
                ...
                try {
                    ...
                    showMessage("@fcf.filetailer.error.notfound", logFilePath);
                    return;
                }
            } else {
                try {
                    ...
                    showMessage("@fcf.filetailer.error.illegalstate", logFilePath);
                    return;
                } catch (FileNotFoundException e) {
                    log.error(e.getMessage(), e);
                    showMessage("@fcf.filetailer.error.notfound", logFilePath);
            ...
            showMessage("@fcf.filetailer.msg.duration", tailer.getMaxActiveInterval() / 1000);
            timer.setInterval(TIMER_INTERVAL);
            timer.start();
            log.trace("Timer started");
            new Thread(tailer).start();
            log.trace("Tailer started");
            btnToggle.setLabel(StrUtil.formatMessage("@fcf.filetailer.btn.toggle.off.label"));
